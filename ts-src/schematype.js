...
const SchemaTypes = {
  ...
  resolve(type) {
    ...
    // SchemaType classes passed directly (e.g. Schema.Types.ObjectId)
    if (type === ObjectIdSchemaType) return ObjectIdSchemaType;
    if (type === StringSchemaType) return StringSchemaType;
    if (type === NumberSchemaType) return NumberSchemaType;
    if (type === BooleanSchemaType) return BooleanSchemaType;
    if (type === DateSchemaType) return DateSchemaType;
    if (type === ArraySchemaType) return ArraySchemaType;
    if (type === MixedSchemaType) return MixedSchemaType;
    if (type === BufferSchemaType) return BufferSchemaType;
    if (type === MapSchemaType) return MapSchemaType;
